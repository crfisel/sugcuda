...
__device__ bool isOppositeSex(int iAgentID, int iMateID, BitWiseType* pbaBits)
{
	return ((&pbaBits[iAgentID])->isFemale != (&pbaBits[iMateID])->isFemale);
}

__device__ bool isFertile(int iAgentID, BitWiseType* pbaBits, short* psaAge, short* psaStartFertilityAge, short* psaEndFertilityAge, 
		float* pfaSugar, float* pfaSpice, float* pfaInitialSugar, float* pfaInitialSpice)
{
	bool ydResult = (psaAge[iAgentID] > ((&pbaBits[iAgentID])->startFertilityAge + 12)) &&
